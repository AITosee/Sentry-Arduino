...
// #define SENTRY_I2C
#define SENTRY_UART
#define VISION_MASK Sentry::kVisionShapeCard
Sentry sentry;

const char* shape_card_classes[] = {
  "unknown",
...
void setup() {
  ...
  printf("sentry.VisionBegin(kVisionShapeCard): %s[0x%x]\n", err ? "Error" : "Success", err);
}
...
void loop() {
  ...
  if (obj_num) {
    ...
    for (int i = 1; i <= obj_num; ++i) {
      ...
      printf("  obj[%d]: x=%d,y=%d,w=%d,h=%d, label=%s\n", i, x, y, w, h, shape_card_classes[l ? l - 10 : l]);
    }
  }
}
